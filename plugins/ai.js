...
smd(
  ...
  async (m) => {
    try {
      ...
      await m.send("Chill lemme think 🤔");
      ...
      const imageUrl = "https://files.catbox.moe/5zxe1k.jpg"; // Replace with your image URL

      // Combine the result and footer in the message
      const message = `*Response:* \n\n${resultText}\n\n*Arised by Phantom*`; // Add footer
  ...
  try {
    ...
    if (contentType && contentType.startsWith('image')) {
      ...
      await m.bot.sendFromUrl(
        m.from,
        photoUrl,
        "Here is your processed photo by Phantom 🐉:",
        m,
        {},
        "image"
      );
    ...
    try {
      ...
      if (contentType && contentType.startsWith('image')) {
        ...
        await m.bot.sendFromUrl(
          m.from,
          photoUrl,
          "*King_Phantom generated this photo 🐉*:",
          m,
          {},
          "image"
        );
    ...
    async (_0x2acf30) => {
      try {
        if (
          ...
          if (
            _0x2acf30.reply_text.includes("KING-PHANTOM • ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            _0x2dfb59[0].includes("KING-PHANTOM • ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            ...
            try {
              if (_0x2ecd2a) {
                ...
                if (_0x13a11c.toLowerCase().startsWith("reply")) {
                  ...
                  let _0x3f6b59 =
                    "*KING-PHANTOM • ʏᴏᴜʀ ᴀɴᴏɴʏ-ᴍsɢ ʀᴇᴘʟʏ*\n\n*_From @" +
        ...
        if (!_0xd9b857.includes("x")) {
          return _0x165087.send(
            "*You did not add x in number.*\n*Ex: " +
              prefix +
              "advt 27748xxxx,Your_Message_here*  \n " +
              Config.caption
          );
        }
...
smd(
  {
    pattern: "phantom",
    desc: "Get a response from PHANTOM-AI based on user query.",
    category: "ai",
    filename: __filename,
  },
  async (m) => {
    try {
      ...
      const query = m.text.split(' ').slice(1).join(' ');
      if (!query) {
        return await m.send("Please provide a query, e.g., `,phantom What is the weather today?`.");
      }

      // Send a loading message
      await m.send("......let him cook🤔");
...
*🐉 Powered by Phantom himself*`;

      // Define the image URL (replace with your desired image URL)
      const imageUrl = "https://files.catbox.moe/5zxe1k.jpg"; // Update with your actual image URL
