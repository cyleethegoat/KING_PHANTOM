...
🐉  **𝞜𝙐𝞛:** ${owner.split(",")[0]} 
🐉  **𝞛𝞢𝞛𝞗:** ${formatp(os.totalmem() - os.freemem())}
🐉  **𝘾𝞗-𝙁𝞗𝙐𝞜𝘿𝞢𝞒𝙎:** _*HAKI, KING DAVID AND PREXZY_*
🐉  **𝙁𝞗𝙐𝞜𝘿𝞢𝞒:** 𝙋𝙃𝘼𝙉𝙏𝙊𝙈
 ᴍᴇɴᴜꜱ 👇👇👇👇👇
...
smd(
  ...
  async (message) => {
    ...
    const { key } = await message.reply("👑 𝙆𝙄𝙉𝙂 𝙋𝙃𝘼𝙉𝙏𝙊𝙈 👑");

    // Wait briefly for effect
    await new Promise((r) => setTimeout(r, 1500));

    var speed = new Date().getTime() - startTime;
    await message.send(`👑 KING_PHANTOM'S SPEED: ${speed} ms ⚡`, { edit: key });
  }
);
astro_patch.cmd(
  {
