...
smd(
  ...
  async (m) => {
    try {
      ...
      await m.send("*_Phantom is writing the lyrics, chill🎶_*");
...
smd({
    ...
    async(message, text) => {
 if(!text) return await message.reply('Give Me Number without + sign. Example: .iswa 27748xxxx')
        var inputnumber = text.split(" ")[0]
        if (!inputnumber.includes('x')) return message.reply(`*You did not add x*\nExample: iswa 27748xxxx  \n ${Config.caption}`)
...
smd({
    ...
    async(message, text) => {
if(!text) return await message.reply('Give Me Number without + sign. Example: .nowa 27748xxxx')
const inputNumber = text.split(" ")[0]
if (!inputNumber.includes('x')) return message.reply(`*You did not add x in number.*\nExample: ${prefix}nowa 27748xxxx  \n ${Config.caption}`)
